...
import uk.me.pausten.yview.model.ServiceCmd;
...
public class JSONProcessor {
	...
	public static String GetServicesList(JSONObject json) {
		...
		//Local Yview (Old) devices don't advertise their services (The services tag is added by the icons gateway)
		else {
			String productID = GetProductID(json);
			if( productID.equals(WYTERM_PRODUCT_ID) ) {
				services = ServiceCmd.HTTP_SERVICE_NAME+":80,"+ServiceCmd.SERIAL_PORT_SERVICE_NAME+":23";
			}
			else if( productID.equals(WYSWITCH2_PRODUCT_ID) ) {
				services = ServiceCmd.HTTP_SERVICE_NAME+":80";
			}
			else if( productID.equals(WYSW_PRODUCT_ID) ) {
				services = ServiceCmd.HTTP_SERVICE_NAME+":80";
			}
			else if( productID.equals(WYGPIO_PRODUCT_ID) ) {
				services = ServiceCmd.HTTP_SERVICE_NAME+":80";
			}
			else if( productID.equals(SERVER_ID) ) {
				services = ServiceCmd.SSH_SERVICE_NAME+":22";
			}
			else if( productID.equals(OGSOLAR) ) {
